...
namespace Editor
{
    ...
    int MapDataInstaller::DownloadPoi(const std::string& sToken, Job::Cmd cmd, ProgressEventer* progress, int nIdx)
    {
        for(int j = 0; j < m_vProjectId.size(); j++)
        {
            ...
            else
            {
                ...
                std::string sUrl = Editor::DataManager::getInstance()->getServiceAddress()->GetPoiDownloadUrl(sToken, m_vProjectId[j], m_sUserId,PU->download_latest, PU->download_latest!="");
    ...
    int MapDataInstaller::POICallBack(Editor::Job* job)
    {
        ...
        if(dataSource->InstallPoi(sFilePath, job->m_sFoundFlag, job->GetJobType(),job->GetPregressObj()) == -1)
        {
            return -1;
        }
...
//		}
	...
	std::string MapDataInstaller::GetDownloadPrepared(const std::string sToken, const std::string sProjectId, const std::string sUserId, const std::string time, int type)
    {
        ...
        {
            std::string longitude = "12";
            std::string radius = "12";
            std::string latitude = "12";
            std::string type = "all";
            ...
            rapidjson::Value longitudeValue(kStringType);
            longitudeValue.SetString(longitude.c_str(), longitude.size(),allocator);
            urlDoc.AddMember("longitude",longitudeValue, allocator);
            
            rapidjson::Value radiusValue(kStringType);
            radiusValue.SetString(radius.c_str(), radius.size(),allocator);
            urlDoc.AddMember("radius",radiusValue,allocator);
            
            rapidjson::Value projectIdValue(kStringType);
            projectIdValue.SetString(sProjectId.c_str(), sProjectId.size(),allocator);
            urlDoc.AddMember("projectId",projectIdValue,allocator);
            
            rapidjson::Value latitudeValue(kStringType);
            latitudeValue.SetString(latitude.c_str(), latitude.size(),allocator);
            urlDoc.AddMember("latitude",latitudeValue,allocator);
            
            rapidjson::Value typeValue(kStringType);
            typeValue.SetString(type.c_str(), type.size(),allocator);
            urlDoc.AddMember("type",typeValue,allocator);
            ...
            sUrl = Editor::DataManager::getInstance()->getServiceAddress()->GetChargingDownloadUrl(sToken, parameter);
            
            if(Tools::HttpGet(sUrl.c_str(),response, errcode) == false)
            {
                Logger::LogO("GetDownloadPrepared : HttpGet [0:poi charging] error!!!");
                Logger::LogD("GetDownloadPrepared : HttpGet [0:poi charging] error!!!");
            }

        }
        else if(type == POI_INFO)
        {
    ...
    void MapDataInstaller::DownloadFile(const std::string& inputJson, ProgressEventer* progress)
    {
        ...
        if(type == POI_CHARGING) //为充电桩时候直接进行安装。
        {
            CallBack(job);
        }
        else
