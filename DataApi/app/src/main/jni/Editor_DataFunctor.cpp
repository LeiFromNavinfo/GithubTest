...
namespace Editor
{
    ...
    int DataFunctor::UpdateFeature(const std::string& tableName, const std::string& json)
    {
        ...
        try
        {
            ...
            JSON oldjson = featureOld->GetTotalPart();
            feature->SetTotalPart(oldjson);
            feature->SetTotalPart(jsonObject);
    ...
    int DataFunctor::DeleteFeatures(const std::string& tableName, std::vector<std::string> ids)
    {
        if(strcmp(tableName.c_str(), "")==0)
        {
            Logger::LogD("DeleteFeatures failed: tablename is invalid");
            Logger::LogO("DeleteFeatures failed: tablename is invalid");
        ...
        if(NULL == dataSource)
        {
            Logger::LogD("DeleteFeatures failed: no such table. Table name: [%s].",tableName.c_str());
            Logger::LogO("DeleteFeatures failed: no such table. Table name: [%s].",tableName.c_str());
        ...
        try
        {
            ...
            if(NULL == layer)
            {
                Logger::LogD("DeleteFeatures failed: no such layer. Table name: [%s].",tableName.c_str());
                Logger::LogO("DeleteFeatures failed: no such layer. Table name: [%s].",tableName.c_str());
                
                return -1;
            }
            
            int batchCount = ids.size()/MaxBatchSize;
            
            if(ids.size()%MaxBatchSize!=0)
            {
                batchCount++;
            }
            ...
            for(int i=0;i<batchCount;i++)
            {
                ...
                if(i== batchCount-1)
                {
                    end = ids.size()%MaxBatchSize==0?(i+1)*MaxBatchSize-1:ids.size()%MaxBatchSize-1;;
                }
                else
                {
                    end = (i+1)*MaxBatchSize-1;
                }
                
                std::vector<std::string> eachBatchIds = Tools::GetSubVector<std::string>(ids, from, end);
                
                std::string inStatement = Tools::GetInStatement(eachBatchIds, 0);
                
                std::string sql = "delete from "+tableName+" where rowid in "+ inStatement;
                
                ret &= layer->ExcuteSql(sql);
            }
        ...
        {
            Logger::LogD("Editor: DeleteFeatures failed, Exception:[%s],type[%s]", e.what(),typeid(e).name());
            Logger::LogO("Editor: DeleteFeatures failed, Exception:[%s],type[%s]", e.what(),typeid(e).name());
