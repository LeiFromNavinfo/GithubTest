...
{	
	...
	int DataLayer::InsertFeature(DataFeature* feature)
	{
		...
		if( result != SQLITE_OK )
		{
            Logger::LogD("DataLayer::InsertFeature failed! sql[%s], err[%s]", sql.c_str(),sqlite3_errmsg(m_Sqlite3Db));
			Logger::LogO("DataLayer::InsertFeature failed! sql[%s], err[%s]", sql.c_str(),sqlite3_errmsg(m_Sqlite3Db));
			return -1;
		}
