...
namespace Model
{
    ...
    int Point::UpdateSqliteTable(sqlite3* db)
    {
	    ...
	    if (rc != SQLITE_OK)
	    {
		    Logger::LogD("update %s failed [%s]", "Point", sqlite3_errmsg(db));
            Logger::LogO("update %s failed [%s]", "Point", sqlite3_errmsg(db));
    ...
    std::string Point::SetPrimaryKey()
    {
        ...
        {
            return Tools::NumberToString(pid) + "1";
        }
        else if(type == 2)
        {
            return Tools::NumberToString(pid) + "2";
        }
        else
        {
            return "";
    ...
    int Infor::InsertSqliteTableWithoutTransaction(sqlite3* db)
    {
        ...
        sqlite3_bind_text(stmt, 1, globalId.c_str(), _id.length(), NULL);
    ...
    int Infor::UpdateSqliteTableWithoutTransaction(sqlite3* db)
    {
	    ...
	    std::string sql = "update edit_infos set "
		    ...
		    "r_similarity=?, f_array=?, geometry=?, task_status=?, submitStatus=? where globalId=?";
        ...
        sqlite3_bind_text(stmt, 39, globalId.c_str(), globalId.length(), NULL);
