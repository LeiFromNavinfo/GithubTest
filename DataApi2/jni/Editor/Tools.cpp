    ...
    bool HttpGet(const char* url, std::string& response)
    {
        ...
        if (curlCode == CURLE_OK)
        {
            ...
            if (resCode != 200)
            {
                ...
                Logger::LogD("HttpGet failed! http response code [%ld]", resCode);
                Logger::LogO("HttpGet failed! http response code [%ld]", resCode);
            }
        }
        else
        {
            ...
            if (len)
            {
                Logger::LogD("HttpGet failed! curl error code [%d], msg [%s]", curlCode, errbuf);
                Logger::LogO("HttpGet failed! curl error code [%d], msg [%s]", curlCode, errbuf);
            }
            else
            {
                Logger::LogD("HttpGet failed! curl error code [%d], msg [%s]", curlCode, curl_easy_strerror(curlCode));
                Logger::LogO("HttpGet failed! curl error code [%d], msg [%s]", curlCode, curl_easy_strerror(curlCode));
            }
        }
    ...
    bool HttpMultiPost(const char* url, char** pName, char** pContents, int iParamLen, std::string& response)
    {
    	...
    	if(curl && multi_handle)
    	{
    	    ...
    	    do
    	    {
    	        ...
    	        if(mc != CURLM_OK)
    	        {
    	        	Logger::LogD("HttpMultiPost, curl_multi_fdset() failed! code [%d]", mc);
    	    	    Logger::LogO("HttpMultiPost, curl_multi_fdset() failed! code [%d]", mc);
