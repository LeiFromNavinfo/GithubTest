...
namespace Editor
{
    ...
    void MapDataPackager::RunPoi(std::string sToken, JOBCMD cmd, ProgressEventer* progress, int nIdx, const std::string& deviceId)
    {
    	...
    	for(int j = 0; j < m_vProjectId.size(); j++)
    	{
            ...
            int upd=0;
            int del=0;
			std::string sDataPath = GetUploadFile(sFilename, PU->upload_latest, errmsg, m_vProjectId[j], total, add, upd, del);
            ...
            if(UploadSnapShot(sDataPath.c_str(), sFilename.c_str(), sToken.c_str(), nFileSize, nChunk) == false)
            {
                return;
            }
            ...
            for(int k = 0; k < nChunk; k++)
            {
                ...
                std::string sUrl = Editor::DataManager::getInstance()->getServiceAddress()->GetPoiUploadDataChunk(sToken, sFilename, k);
    ...
    void MapDataPackager::RunInfor(std::string sToken, JOBCMD cmd, ProgressEventer* progress, int nIdx)
    {
        ...
        for(int j = 0; j < m_vProjectId.size(); j++)
        {
            ...
            std::string sUploadFilePath = GetUploadInfoFile(sFilename, errmsg, sPackageSql);
            ...
            if(UploadSnapShot(sUploadFilePath.c_str(), sFilename.c_str(), sToken.c_str(), nFileSize, nChunk) == false)
            {
                return;
            }
    ...
    void MapDataPackager::PoiCallBack(Editor::Job* job)
    {
        ...
        {
            if(UploadCheck((job->m_sToken).c_str(), (job->m_sFileName).c_str()) == false)
            {
                Locker::UnLock();
    ...
    void MapDataPackager::InforCallBack(Editor::Job* job)
    {
        ...
        if(m_nSuccessChunkNum == m_nChunkNum)
        {
		    ...
		    if(SaveToMainDBAfterUpload(job, "infor") == true)
		    {
                ...
                std::string sql = "update edit_infos set submitStatus = 1;";
    ...
    std::string MapDataPackager::GetUploadFile(std::string& sFilename, std::string sLastTime, std::string& errmsg, const std::string& sProjectId, int& total, int& add, int& upd, int& del)
    {
        std::string sFilePath = GenerateUploadFile(sFilename, sLastTime, errmsg, sProjectId, total, add, upd, del);
	...
	std::string MapDataPackager::GetUploadInfoFile(std::string& sFilename, std::string& errmsg, const std::string& sPackageSql)
    {
        std::string sFilePath = GenerateUploadFile(sFilename, errmsg, sPackageSql);
    ...
    std::string MapDataPackager::GenerateUploadFile(std::string& sFilename, std::string sLastTime, std::string& errmsg, const std::string& sProjectId, int& total, int& add, int& upd, int& del)
    {
        ...
        std::string sql = "select * from edit_pois where lifecycle !=0 and project = '" + sProjectId + "'";
        ...
        for(int i=0; i< pois.size(); i++)
        {
            ...
            {
                std::string sTmp = pois.at(i)->ToUploadJsonStr();
    ...
    std::string MapDataPackager::GenerateUploadFile(std::string& sFilename, std::string& errmsg, const std::string& packageSql)
    {
        /*std::string sData = m_pDataFileSystem->GetCoreMapDataPath();
        ...
        {
            std::string sTmp = infos.at(i)->ToUploadJsonStr();
...
//        m_pDataUploader->StartJob(job, NULL, progress);
    ...
    bool MapDataPackager::UploadSnapShot(const char* target, const char* fileName, const char* token, unsigned long& fileSize, int& chunkCount)
    {
        ...
        std::string sSnapshot = Editor::DataManager::getInstance()->getServiceAddress()->GetPoiUploadSnapShot(token, fileName, md5Context.hex_digest(), fileSize);
    ...
    bool MapDataPackager::UploadCheck(const char* token, const char* sFilename)
    {
        std::string sCheck = Editor::DataManager::getInstance()->getServiceAddress()->GetPoiUploadCheck(token, sFilename);
    ...
    bool MapDataPackager::SaveToMainDBAfterUpload(Editor::Job* job, const std::string type)
    {
        ...
        if(type == "poi")
        {
            sExcuteUrl = Editor::DataManager::getInstance()->getServiceAddress()->GetPoiUploadExcuteUrl(job->m_sToken);
    ...
    std::string MapDataPackager::GetUploadPrepared(const std::string& sToken, const std::string sProjectId, const std::string& sDeviceId, int type)
    {
        ...
        if(type == 0)
        {
		    ...
		    std::string sDataPath = GetUploadFile(sFilename, time, errmsg, sProjectId, total, add, upd, del);
        ...
        if(type == 1)
        {
            std::string sPackageSql = "select * from edit_infos;";
            ...
            {
                Logger::LogD("GetUploadPrepared : package info error!!!");
                Logger::LogO("GetUploadPrepared : package info error!!!");
    ...
    void MapDataPackager::UploadFile(const std::string& input, ProgressEventer* progress)
    {
        ...
        if(document.HasMember("projectid"))
        {
            sProjectId = document["projectid"].GetString();
        }
        ...
        if(UploadSnapShot(sPath.c_str(), sFilename.c_str(), sToken.c_str(), nFileSize, nChunk) == false)
        {
            return;
        }
