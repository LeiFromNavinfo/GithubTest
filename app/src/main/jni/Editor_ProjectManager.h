...
namespace Editor 
{
    ...
    typedef enum BASIC_TYPE
    {
        ...
        PatternImage = 4,   //pattrenImage
        
        ROAD_INFO = 5,      //道路情报
        
        POI_CHARGING = 6,   //充电桩充电站
    ...
    private:
        ...
        std::string             GetUploadFile(std::string& sFilename, const std::string sLastTime, std::string& errmsg, const std::string& sProjectId, int& total, int& add, int& upd, int& del, int type);

        std::string             GetUploadInfoFile(std::string& sFilename, std::string& errmsg, const std::string& sPackageSql,const std::string& sProjectId,int type);

        std::string             GenerateUploadFile(std::string& sFilename, const std::string sLastTime, std::string& errmsg, const std::string& sProjectId, int& total, int& add, int& upd, int& del, int type);

        std::string             GenerateUploadFile(std::string& sFilename, std::string& errmsg, const std::string& packageSql, int type);
        ...
        bool                    UploadSnapShot(const char* target, const char* fileName, const char* token, unsigned long& fileSize, int& chunkCount, int type);

        bool                    UploadCheck(const char* token, const char* sFilename, int type);
