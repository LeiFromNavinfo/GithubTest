...
namespace DataTransfor
{
    ...
    std::string GetPoiGlobalId(rapidjson::Document& doc)
    {
        ...
        {
            globalId = Tools::NumberToString(pid)+Tools::NumberToString(type);
        }
    ...
    std::string GetPoiDisplayStyle(rapidjson::Document& doc)
    {
        ...
        int lifecycle,qtStatus,evaluatePlanning,type;
        ...
        if(type == 1 || type == 21) //删除标记和大门
            displaystyle = Tools::NumberToString(type);
        else
        {
    ...
    std::string GetTipsDisplayStyle(const std::string& tipsJson)
	{
        ...
        case Model::RealImage:
            {
                iconStyleString = GetRealImageDisplayStyle(deepDoc);
    ...
    std::string GetRealImageDisplayStyle(rapidjson::Document& doc)
    {
        rapidjson::Document::MemberIterator member_iterator = doc.FindMember("tp");
        ...
        int tp = valueKey.GetInt();
        
        std::string iconName = Tools::NumberToString(Model::HighwayBranch) + ResourceSplitSymbol + Tools::NumberToString(tp) + ResourceSplitSymbol +"0";
...
EDITOR_API std::string Points2Wkt( std::vector<EditorGeometry::Point2D>& points, int geoType )
{
    ...
    EDITOR_API std::string GetInfoGlobalId(rapidjson::Document& doc)
    {
        if(doc.HasParseError() || !doc.HasMember(Info_bfeatureKind.c_str()) ||!doc.HasMember(Info_bsourceCode.c_str()) ||
           !doc.HasMember(Info_bsourceId.c_str()))
        {
            Logger::LogD ("GetInfoGlobalId::Parse poi failed");
            Logger::LogO ("GetInfoGlobalId::Parse poi failed");
            return "";
        }
        
        int featureKind,sourceCode;
        
        std::string sourceId,featureKindStr,sourceCodeStr;
        
        if(!doc[Info_bfeatureKind.c_str()].IsNull())
        {
            featureKind = doc[Info_bfeatureKind.c_str()].GetInt();
        }
        
        if(!doc[Info_bsourceCode.c_str()].IsNull())
        {
            sourceCode = doc[Info_bsourceCode.c_str()].GetInt();
        }
        
        if(!doc[Info_bsourceId.c_str()].IsNull())
        {
            sourceId = doc[Info_bsourceId.c_str()].GetString();
        }
        
        if(featureKind>=0 &&featureKind <=9)
        {
            featureKindStr = "0"+Tools::NumberToString(featureKind);
        }
        
        if(sourceCode>=0 && sourceCode<=9)
        {
            sourceCodeStr = "0"+Tools::NumberToString(sourceCode);
        }
        
        std::string globalId = featureKindStr+sourceCodeStr+sourceId;
        
        return globalId;
    }
    
    EDITOR_API std::string GetInfoDisplayStyle(rapidjson::Document& doc)
    {
        if(doc.HasParseError() || !doc.HasMember(Info_bfeatureKind.c_str()) ||!doc.HasMember(Info_bsourceCode.c_str()) ||
           !doc.HasMember(Info_bsourceId.c_str())||
           !doc.HasMember(Info_level.c_str()))
        {
            Logger::LogD ("GetInfoDisplayStyle::Parse doc failed");
            Logger::LogO ("GetInfoDisplayStyle::Parse doc failed");
            return "";
        }
        
        int featureKind=0,sourceCode=0, isAdopted=0, iLevel=0;
        
        if(!doc[Info_bfeatureKind.c_str()].IsNull())
        {
            if (doc[Info_bfeatureKind.c_str()].IsInt())
            {
                featureKind = doc[Info_bfeatureKind.c_str()].GetInt();
            }
            else if (doc[Info_bfeatureKind.c_str()].IsDouble())
            {
                featureKind = doc[Info_bfeatureKind.c_str()].GetDouble();
            }
        }
        ...
        if(doc.HasMember(Info_isAdopted.c_str()) && !doc[Info_isAdopted.c_str()].IsNull())
        {
            isAdopted = doc[Info_isAdopted.c_str()].GetInt();
        }
        
        if(!doc[Info_level.c_str()].IsNull())
        {
            iLevel = doc[Info_level.c_str()].GetInt();
        }
        ...
        if(sourceCode==1)
        {
            bit1= isAdopted==0?0:2;
            
            bit2=iLevel>2?1:0;
        }
        else if(sourceCode==2)
        {
            bit1 = featureKind%2==0?1:0;
            
            bit2= featureKind>2?1:0;
        }
        
        std::string displayStyle = Tools::NumberToString(sourceCode)+"_"+Tools::NumberToString(bit1)+"_"+Tools::NumberToString(bit2)+"";
