...
namespace Editor 
{
    ...
    typedef enum Editor_BSourceCode2InfoType
    {
        Editor_BSourceCode2InfoType_UnknownInfo=0,
        Editor_BSourceCode2InfoType_PoiInfo=1,
        Editor_BSourceCode2InfoType_RoadInfo=2
        
    }Editor_BSourceCode2InfoType;
    ...
    class Editor_TrackStatisticsFieldsName
    {
    public:
        std::string userIdFieldNmae;
        std::string datetimeFieldNmae;
    };
    ...
    struct tagTrackforServer
    {
        std::string strID;
        std::string userID;
        std::string trackdate;
        std::vector<geos::geom::Geometry* > trackGeos;
    };
    
    struct tagSyncTrackPara
    {
        std::string sToken;
        std::string sUserID;
        std::string sProjectID;
        Editor::ProgressEventer* pregress;
    };
    ...
    public:
        ...
        std::string     GetChargingDownloadUrl(const std::string& token, const std::string& parameter);

        std::string		GetPoiUploadUrl(std::string sFileName, std::string sProject);

        std::string     GetPoiUploadExcuteUrl(const std::string& token,int type);

        std::string     GetInforUploadExcuteUrl(const std::string sToken, const std::string sFileName);
        std::string     GetPoiUploadSnapShot(const std::string& token, const std::string& fileName, const std::string& md5, unsigned long fileSize, int type);

        std::string     GetPoiUploadDataChunk(const std::string& token, const std::string& fileName, int chunkNo, int type);

        std::string     GetPoiUploadCheck(const std::string& token, const std::string& fileName, int type);
        ...
        std::string     GetUploadTrackUrl();
        std::string     GetTrackImportExcuteUrl(const std::string& sToken, const std::string& parameter);
    private:
    ...
    public:
        ...
        virtual int                         InstallPoi(std::string sFilePath, std::string sFlag, int type, ProgressEventer* progress);
 ...
 class EDITOR_API Matcher
    ...
    public:
        ...
        //根据rowid列表删除记录
        int                                       DeleteFeatures(const std::string& tableName, std::vector<std::string> ids);
    };
}
