...
//		}
    ...
    int CoreMapDataSource::InstallPoi(std::string sFilePath, std::string sFlag, int type,ProgressEventer* progress)
    {
        ...
        std::string sPathFolderFileTmp;
        std::string sPathFolderTmp;
        if(type == POI_CHARGING) //为充电桩时候。
        {
            sPathFolderFileTmp = sFilePath;
        }
        else
        {
            ...
            if(!(nBeg != std::string::npos && nBeg == sFilePath.length() - 1))
            {
             ...
             sPathFolderTmp = sFilePath.substr(0, sFilePath.length() - 4);
            
#ifdef WIN32
            sPathFolderFileTmp = sPathFolderTmp + "\\tmp.txt";
#else
            sPathFolderFileTmp = sPathFolderTmp + "/tmp.txt";
#endif
        ...
        DataTransfor::ExecuteNonQuery("PRAGMA temp_store_directory = '/storage/extSdCard'", hDB);
	...
	int CoreMapDataSource::InstallInfo(std::string sFilePath, std::string sFlag, ProgressEventer* progress)
    {
        try
        {
            Logger::LogD("CoreMapDataSource::InstallInfo begin!!!\n");
            Logger::LogO("CoreMapDataSource::InstallInfo begin!!!\n");
            ...
            while(getline(in, line))
            {
                ...
                Logger::LogD("edit_infos json:[%s],cn[%d]\n", line.c_str(),++cn);
                Logger::LogO("edit_infos json:[%s],cn[%d]\n", line.c_str(),++cn);
               ...
               if(info.h_projectId=="")
                ...
                if(info.b_sourceCode!=int(Editor_BSourceCode2InfoType_PoiInfo)
                   ||info.b_sourceId==""
                   ||info.globalId=="") continue;

                if(IsExistsInDatabase(hDB, "edit_infos", "globalId", info.globalId) == 0)
                {
                    Logger::LogD("Update edit_infos line:[%s]\n", line.c_str());
                    Logger::LogO("Update edit_infos line:[%s]\n", line.c_str());
                    Logger::LogD("Update edit_infos globalId:[%s],_id[%s]\n", info.globalId.c_str(),info._id.c_str());
                    Logger::LogO("Update edit_infos globalId:[%s],_id[%s]\n", info.globalId.c_str(),info._id.c_str());
                    info.UpdateSqliteTableWithoutTransaction(hDB);
                }
                else
                {
                    Logger::LogD("Insert edit_infos line:[%s]\n", line.c_str());
                    Logger::LogO("Insert edit_infos line:[%s]\n", line.c_str());
                    Logger::LogD("Insert edit_infos globalId:[%s],_id[%s]\n", info.globalId.c_str(),info._id.c_str());
                    Logger::LogO("Insert edit_infos globalId:[%s],_id[%s]\n", info.globalId.c_str(),info._id.c_str());
            ...
            Logger::LogD("CoreMapDataSource::InstallInfo end!!!\n");
            Logger::LogO("CoreMapDataSource::InstallInfo end!!!\n");
        ...
        {
            Logger::LogD("CoreMapDataSource::InstallInfo failed, Exception:[%s],type[%s]", e.what(),typeid(e).name());
            Logger::LogO("CoreMapDataSource::InstallInfo failed, Exception:[%s],type[%s]", e.what(),typeid(e).name());
